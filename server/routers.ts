  ...
  bot: router({
    ...
    start: publicProcedure
      .input(z.object({
        ...
        maxTradesPerDay: z.number().default(0),
       ...
       if (input.instrumentToken.startsWith("MCX_FO|")) {
        ...
        if (existing.length > 0) {
          ...
          await db
            ...
            .set({
              ...
              enabledLayers: JSON.stringify((input.enabledLayers ?? []).length ? input.enabledLayers : getRecommendedLayers(input.instrumentLabel)),
        ...
        const onTradeClose = async (
          ...
          const entryRow = await dbInner
            .select({ entryPrice: tradeLog.entryPrice, quantity: tradeLog.quantity, enteredAt: tradeLog.enteredAt })
            .from(tradeLog)
  ...
  trades: router({
    ...
    correctTradeExit: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (authTok) {
          try {
            const dec = jwt.verify(authTok, getJwtSecret()) as { mobile?: string; role?: string };
      ...
      .query(async ({ input, ctx }) => {
        ...
        const slotResults = slotTokens.map(tok => {
          ...
          return {
            ...
            // Current option premium price is valid only with explicit quote health.
            optionPremiumPrice: inMem?.optionQuoteStatus !== "unavailable"
              ? (inMem?.optionPremiumPrice ?? null)
              : null,
            optionQuoteStatus: (inMem?.isIndexOptions ?? dbRow?.isIndexOptions)
      ...
      if (!slotAccess.hasAccess && !isSlotAdminSession && !isSlotAdminViaCookie) {
        ...
        if (existing.length > 0) {
          ...
          await db.update(botSessions).set({
            ...
            tradesCount: slotTodayCount, dailyPnl: 0,
          ...
          const result = await db.insert(botSessions).values({
            ...
            botSlot: input.slot, tradesCount: slotTodayCount, dailyPnl: 0,
            isIndexOptions: input.isIndexOptions ?? false,
            underlyingToken: input.underlyingToken ?? null,
            startedAt: new Date(),
            enabledLayers: JSON.stringify(input.enabledLayers),
        ...
        const startResult = startBot({
          ...
          targetMultiplier: input.targetMultiplier, trailingSlEnabled: input.trailingSlEnabled,
          trailingSlPct: input.trailingSlPct, minConfidence: input.minConfidence,
          scanIntervalSec: input.scanIntervalSec, tradesCount: slotTodayCount, dailyPnl: 0, accessToken,
          telegramBotToken: input.telegramBotToken ?? null,
          telegramChatId: input.telegramChatId ?? null,
          telegramEnabled: input.telegramEnabled, botSlot: input.slot,
    ...
    scanStrikes: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        try {
          ...
          if (quoteRes.ok) {
            const qJson = await quoteRes.json() as { data?: Record<string, { instrument_token?: string; last_price?: number }> };
            const quote = selectRequestedUpstoxQuote(qJson.data, underlyingToken);
            underlyingPrice = quote?.last_price ?? 0;
          }
        } catch { /* ignore */ }
        ...
        // Get the nearest option chain using exact upcoming dates from the
        // authoritative contract list. Never request aliases or accept stale rows.
        try {
          const expiryOrder = await fetchUpcomingOptionExpiryKeys(underlyingToken, accessToken, 4);
          if (expiryOrder.length === 0) {
            return { candidates: [], expiryDate: null, underlyingPrice, scanTime: Date.now(), error: "No unexpired option contract found" };
          }

          type HeroZeroChainRow = {
            expiry?: string;
            pcr?: number;
            strike_price?: number;
          ...
          let chainData: HeroZeroChainRow[] = [];
          let expiryDate: string | null = null;
          for (const expiry of expiryOrder) {
            const chainRes = await upstoxFetch(`https://api.upstox.com/v2/option/chain?instrument_key=${encodeURIComponent(underlyingToken)}&expiry_date=${expiry}`, {
              headers: { Authorization: `Bearer ${accessToken}`, Accept: "application/json" },
              signal: AbortSignal.timeout(10000),
            });
            if (!chainRes.ok) {
              const errText = await chainRes.text().catch(() => "");
              console.error(`[HeroZero] Option chain fetch failed for ${expiry}:`, chainRes.status, errText);
              continue;
            }
            const chainJson = await chainRes.json() as { data?: HeroZeroChainRow[] };
            const tradableRows = (chainJson.data ?? []).filter(row =>
              getOptionExpiryDateKey(row.expiry) === expiry && isOptionExpiryTradable(row.expiry),
            );
          ...
          if (chainData.length === 0 || !expiryDate) {
            return { candidates: [], expiryDate: null, underlyingPrice, scanTime: Date.now(), error: "No valid unexpired option chain returned" };
          }
    ...
    smartScan: publicProcedure
      ...
      .query(async ({ input }) => {
        const SCAN_INSTRUMENTS = [
          ...
          { token: "MCX_FO|563946",               symbol: "MCX_GOLD",  label: "Gold → ATM Options",        lotSize: 100 },
  ...
  subscription: router({
    ...
    checkAccess: publicProcedure
      ...
      .query(async ({ input, ctx }) => {
        ...
        if (adminToken) {
          try {
            const decoded = jwt.verify(adminToken, getJwtSecret()) as { userId: number; mobile: string; role: string };
  ...
  mobileAuth: router({
    ...
    verifyOtp: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // Issue a 30-day token using the one persistent Railway signing secret.
...
me: publicProcedure
    ...
    updateName: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const decoded = verifyMobileAuthToken(token);
        const db = await getDb();
        if (!db) throw new Error("DB unavailable");
  ...
  admin: router({
    login: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // Admin sessions use the same persistent 30-day signing contract.
