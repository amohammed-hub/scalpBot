...
export async function sendTelegramAlert(state: BotState, message: string, category?: AlertCategory): Promise {
...
export async function sendTelegramMessage(botToken: string, chatId: string, message: string): Promise {
...
return { ...hold, entryPrice: price, reason: `[MR-V13] BB lower touched but RSI=${rsi.toFixed(1)} not oversold ( }
...
const fullQuoteInflight = new Map<string, Promise>();
...
const requestQuote = async (): Promise => {
const encoded = encodeURIComponent(instrumentToken);
...
// Use fallback price from candles if market-quote failed
if (!underlyingPrice && fallbackPrice > 0) {
underlyingPrice = fallbackPrice;
console.log(`[BotEngine] resolveAtmMcxOptionToken: using fallback candle price ₹${fallbackPrice} for ATM matching`);
}
if (!underlyingPrice) {
console.warn(`[BotEngine] resolveAtmMcxOptionToken: no price available for ${futuresToken} (quote failed + no fallback)`);
return null;
}
...
// Try to fetch live premiums (requires valid access token)
...
console.warn(`[BotEngine] MCX option premium fetch failed (token may be expired): ${quoteFetchErr instanceof Error ? quoteFetchErr.message : String(quoteFetchErr)}`);
}
// If premium fetch succeeded, pick the first strike with premium > threshold
if (quoteFetchOk) {
for (const c of near) {
...
let liveOrderHeaders: Awaited<ReturnType>;
try {
...
// Order status verification (prevents ghost trades)
// After placing a live order, Upstox may accept it (return orderId) but the
// exchange can reject it asynchronously. This function checks the actual status.
...
// Upstox order statuses: "complete", "rejected", "cancelled", "open", "pending", "trigger pending"
const status = (order.status ?? order.order_status ?? "unknown").toLowerCase();
const rejectionReason = order.status_message ?? order.rejection_reason ?? undefined;
const filledQty = order.filled_quantity ?? order.traded_quantity ?? 0;
const avgPrice = order.average_price ?? order.traded_price ?? 0;
return { status, rejectionReason, filledQty, avgPrice };
} catch (err) {
const msg = err instanceof Error ? err.message : String(err);
console.warn(`[BotEngine] verifyUpstoxOrderStatus failed for ${orderId}: ${msg}`);
...
export async function resolveNseLotSize(underlyingToken: string, accessToken: string): Promise {
...
async function tick(
state: BotState,
onTradeOpen: (trade: TradeInsert) => Promise,
...
`📊 ${trade.symbolLabel} | Exit: ₹${exitPx.toFixed(2)}\n` +
...
sendTelegramAlert(state,
`⏰ AUTO SQUARE-OFF (market closing)\n` +
`📊 ${trade.symbolLabel} | Exit: ₹${effectivePrice.toFixed(2)}\n` +
...
// Send Telegram alert
const avgMsg = `📊 MANUAL AVERAGE DOWN\n` +
`📈 ${trade.symbolLabel}\n` +
`➕ Added ${avgQty} qty @ ₹${avgPrice.toFixed(2)}\n` +
