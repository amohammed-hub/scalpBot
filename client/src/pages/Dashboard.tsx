     ...
     {/* ── Trial/Plan Banner ────────────────────────────────────────────────── */}
      ...
      {tokenStatus !== "valid" && config.mode === "live" && !showReminder && (
        ...
        <div data-tour="bot-slots" className={`grid gap-2 sm:gap-3 mb-6 grid-cols-1 sm:grid-cols-2 ${(() => {
          const totalSlots = isAdmin ? Math.max(3, (accessQuery.data as any)?.extraBotSlots ?? 3) : 3 + ((accessQuery.data as any)?.extraBotSlots ?? 0);
          ...
          {((allBots && allBots.length > 0) ? allBots.slice(0, isAdmin ? Math.max(3, (accessQuery.data as any)?.extraBotSlots ?? 3) : 3 + ((accessQuery.data as any)?.extraBotSlots ?? 0)) : (() => {
            const totalSlots = isAdmin ? Math.max(3, (accessQuery.data as any)?.extraBotSlots ?? 3) : 3 + ((accessQuery.data as any)?.extraBotSlots ?? 0);
            const slots = [];
