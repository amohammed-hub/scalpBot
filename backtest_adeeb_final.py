"""
Adeeb Strategy Backtest — FINAL VERSION

Key findings from optimization:
- V3 analysis showed EMA cloud break exit caused ALL 12 of its trades to be losses (-396 pts)
- Without EMA exit: 45 trades, 62.2% WR, PF 2.27, +443 pts
- Max hold trades alone: PF 3.23, 65% WR

FINAL PARAMETERS:
- Entry: ADX>25(BUY)/ADX>30(SELL) + CPR bias + 3-brick Renko + EMA cloud pullback
- Exit: Max hold 30 min + Target (2×ATR) + SL (cloud edge - 0.3×ATR) + Trailing @+1ATR
- NO EMA cloud break exit (causes premature exits in volatile markets)
- Anti-chase: 0.25%
- RSI confirmation: BUY>42, SELL<58
"""
...
print("⚡ Adeeb Strategy — Final Backtest")
print("Fetching NIFTY 50 (^NSEI) 5-min data from Yahoo Finance...")
...
for di in range(1, len(days)):
    ...
    for i in range(SKIP_S, len(C)-SKIP_E):
        ...
        if ot:
            ...
            ex = False; er = ""
            if ch >= MAX_HOLD: ex=True; er="Max hold (30 min)"
        ...
        if dt >= MAX_DAILY or i < 21: continue
...
# ── Print Results ─────────────────────────────────────────────────────────────
print("="*70)
print("⚡ ADEEB STRATEGY — FINAL BACKTEST RESULTS")
print("="*70)
print(f"Data: NIFTY 50 (^NSEI) | 5-min | {len(days)} days")
print(f"Period: {days[0]} to {days[-1]}")
print(f"Exit: Max hold 30min + Target 2×ATR + SL + Trailing @+1ATR (NO EMA exit)")
print("-"*70)
...
if not trades:
    ...
    print(f"")
    print(f"Total P&L:          {total_pnl:+.2f} pts (NIFTY index points)")
    ...
    for i, (date, pnl) in enumerate(sorted(daily_pnl.items())):
        cum_pnl = sum(list(daily_pnl.values())[:i+1])
        bar = "█" * max(1, int(abs(cum_pnl) / 20))
        print(f"    {date} {cum_pnl:+8.2f} {'📈' if cum_pnl > 0 else '📉'} {bar}")
    ...
    with open("backtest_adeeb_final_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nSaved to backtest_adeeb_final_results.json")
